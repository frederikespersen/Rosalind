...
def generate_best_reversals(sequences: list, target_sequence: list) -> list:
    ...
    return [rev.split(' ') for rev in set([' '.join(rev) for rev in best_reversals])]


def reversal_distance(permutations: tuple) -> tuple:
...
if __name__ == '__main__':
    ...
    rd, prs = reversal_distance(pairs[0])

    collection = []
    r = rd
    target = prs[str(r)][0]
    while r > 0:
        for rev in prs[str(r-1)]:
            bps = breakpoints(rev, target)
            if len(bps) == 2:  # I.e. one reversal difference
                collection.append([bps[0], bps[1]-1])  # Correct indeces for Rosalind answer format (1-indexed)
                target = rev
                r -= 1
                break
    collection.reverse()
